...
- **Thumbnails**: At upload time, **3 frames are auto-extracted** from the picked video (Start / Middle / End) using `expo-video-thumbnails` client-side. For **already-uploaded videos**, the Edit Video screen ("Auto-generate from video") calls a new `GET /api/videos/{id}/thumbnail-options` server-side endpoint that pulls the video from R2 via a 5-min presigned URL and runs `ffmpeg`/`ffprobe` to extract 3 JPEG frames (creator-only, ~720p, q=5). Tapping a candidate calls `PUT /api/videos/{id}/thumbnail` to persist it. Users can also upload a custom 16:9 image. Served by existing `GET /api/videos/{id}/thumbnail` (1-hour cache).
